(function(){try {
  ...
  function loadiframe() {
    ...
    iframe.setAttribute('src', '<%-URL_SCHEME%>://<%-CONSENT_SERVER_HOST%>/<%-API_VERSION%>/iframe<%-BANNER%>.html' + (channelId !== '' ? '?channelId=' + channelId : ''));
    ...
    iframe.addEventListener('load', function() {
      window.__tcfapi = function(cmd, ver, cb, parameter) {
        ...
        message('cmd;' + cmd + ';' + ver + ';' + parameter, function(r, s) {cb && cb(r, s)});
      };
  ...
  if (window.navigator && navigator.userAgent && navigator.userAgent.indexOf &&
    ...
    a.setAttribute('src', '<%-URL_SCHEME%>://<%-CONSENT_SERVER_HOST%>/<%-API_VERSION%>/manager<%-BANNER%>.js?<%-TECH_COOKIE_NAME%>=' + xt + (hasConsent !== null ? '&consent=' + hasConsent : '') + (channelId !== '' ? '&channelId=' + channelId : ''));
